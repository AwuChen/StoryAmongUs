...
var Unity_MediaNetwork =
{
	...
	*/
	...
	{
		return awrtc.CAPI_VideoInput_AddCanvasDevice(Pointer_stringify(query), Pointer_stringify(name), width, height, fps);
	},
	...
	{
		awrtc.CAPI_VideoInput_AddDevice(Pointer_stringify(name), width, height, fps);
	},
	//export function CAPI_VideoInput_RemoveDevice(name:string);
	Unity_VideoInput_RemoveDevice: function()
	{
		awrtc.CAPI_VideoInput_RemoveDevice(Pointer_stringify(name));
	},
	...
	{
		return awrtc.CAPI_VideoInput_UpdateFrame(Pointer_stringify(name), HEAPU8, lBufferUint8ArrayPtr + lBufferUint8ArrayOffset, lBufferUint8ArrayLength, width, height, rotation, firstRowIsBottom);
	}
}
