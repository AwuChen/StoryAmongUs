...
namespace Byn.Awrtc.Unity
{
    ...
    {
        private static readonly bool EXPERIMENT_MAINTAIN_RESOLUTION = false;
        ...
        /// trystworthy. 
        /// true = Invalid certificates are ignored. Communication is still encrypted but man in the middle attacks are possile
        ...
        /// </summary>
        public static readonly LogLevel DEFAULT_LOG_LEVEL = LogLevel.Info;
        ...
        /// </summary>
        public static readonly bool NATIVE_VERBOSE_LOG = true;
        ...
        private void InitObj()
        {
            ...
            if (NATIVE_VERBOSE_LOG)
            {
                ...
                //routing to unity log. currently unavailable
                //Awrtc.Native.NativeWebRtcCallFactory.SetNativeLogToSLog(WebRtcCSharp.LoggingSeverity.LS_INFO);
            }
            else
            {
            ...
            //set this to false if you experience problems with video encoding / decoding
            ...
            WebRtcCSharp.IosHelper.PreferH264(true);
#endif
        ...
        private static bool StaticInitBrowser()
        {
            ...
            if (Byn.Awrtc.Browser.CAPI.Unity_MediaNetwork_HasUserMedia() == false)
            {
                ...
                Debug.LogWarning("Browser does not allow access to navigator.mediaDevices. Local camera & microphone won't work." 
                    + "This can be caused by user settings, plugins or extentions! Some browsers (Chrome) only allow access to user media if the page is loaded using HTTPS / WSS only");
            }
