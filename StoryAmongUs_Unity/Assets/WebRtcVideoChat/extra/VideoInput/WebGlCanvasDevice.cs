...
public class WebGlCanvasDevice : MonoBehaviour
{
    ...
    //"#myid" to query the webpage for a specific element with the id "myid"
    ...
    /// 
    /// If set to false:
    ///     The frame will be accessed & scaled durng unity's Update loop. This only works if Unity has the 
    ...
    {
        //Best to run this only if this device is actually used. To keep the test app itself
        //free from any WebGL specific code we
        while (true)
        {
